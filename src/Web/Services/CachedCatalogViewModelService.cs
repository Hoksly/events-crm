...
public class CachedCatalogViewModelService : ICatalogViewModelService
{
    ...
    public async Task<CatalogIndexViewModel> GetCatalogItems(int pageIndex, int itemsPage, List<LocationViewModel>? locationsSelected, List<TypeViewModel>? typesSelected)
    {
        var cacheKey = CacheHelpers.GenerateCatalogItemCacheKey(pageIndex, Constants.ITEMS_PER_PAGE, locationsSelected, typesSelected);

        return (await _cache.GetOrCreateAsync(cacheKey, async entry =>
        {
            entry.SlidingExpiration = CacheHelpers.DefaultCacheDuration;
            return await _catalogViewModelService.GetCatalogItems(pageIndex, itemsPage, locationsSelected, typesSelected);
    ...
    {
        return (await _cache.GetOrCreateAsync(CacheHelpers.GenerateLocationsCacheKey(), async entry =>
        {
            entry.SlidingExpiration = CacheHelpers.DefaultCacheDuration;
            return await _catalogViewModelService.GetLocations();
